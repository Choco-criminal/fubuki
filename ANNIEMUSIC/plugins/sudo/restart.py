...
@app.on_message(filters.command(["shutdown"]) & SUDOERS)
async def shutdown_(_, message):
    response = await message.reply_text("sʜᴜᴛᴛɪɴɢ ᴅᴏᴡɴ...")
    ac_chats = await get_active_chats()
    for x in ac_chats:
        try:
            await app.send_message(
                chat_id=int(x),
                text=f"{app.mention} ɪs sʜᴜᴛᴛɪɴɢ ᴅᴏᴡɴ...\n\nʏᴏᴜ ᴄᴀɴ'ᴛ ᴜsᴇ ᴛʜᴇ ʙᴏᴛ ᴜɴᴛɪʟ ɪᴛ ɪs sᴛᴀʀᴛᴇᴅ ᴀɢᴀɪɴ.",
            )
            await remove_active_chat(x)
            await remove_active_video_chat(x)
        except:
    ...
    try:
        ...
        pass
    await response.edit_text(
        "» sʜᴜᴛᴅᴏᴡɴ ᴘʀᴏᴄᴇss sᴛᴀʀᴛᴇᴅ, ᴛʜᴇ ʙᴏᴛ ᴡɪʟʟ ʙᴇ ᴏғғʟɪɴᴇ sᴏᴏɴ."
    )
